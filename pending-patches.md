...
Background: `--info` was used in TASK_STATUS_COLORS for `in_progress` status 
and `--danger` was used for `deleted` status in formatTaskStatusBadge(), but 
neither was declared in the CSS custom properties. This meant in_progress tasks 
showed no color in tool call rendering.

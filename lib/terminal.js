...
function createTerminalService(o) {
  ...
  function spawnSession(id, mode, cwdCandidate, cols, rows, spec, extraEnv) {
    ...
    const env = { ...ptyEnv({ claudeDir: o.claudeDir, isDefaultDir: o.isDefaultDir, noFlicker: config.noFlicker }), ...extraEnv };
  ...
  function startNew(msg, extraEnv) {
    ...
    const r = start(crypto.randomUUID(), 'new', msg, 120, 32, extraEnv);
  ...
  return { token, handleUpgrade, clientConfig, list, isRunning, end, authorized, startNew, shutdown, unavailableReason };
}

module.exports = { createTerminalService, readTerminalConfig, shellArgs, resolveShell, claudeArgsFor, parseNewSpec, findPickProcess, tokenMatches };

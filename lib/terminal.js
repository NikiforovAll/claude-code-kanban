...
function readTerminalConfig({ argv = process.argv, env = process.env, getArgValue } = {}) {
  ...
  return {
    ...
    shell:
      (getArgValue && getArgValue('terminal-shell')) ||
      env.CCK_TERMINAL_SHELL ||
      (typeof cfg.shell === 'string' && cfg.shell) ||
      null,
    maxSessions: posInt(cfg.maxSessions, 20),
...
function findPickProcess(live, pty, claimed) {
  return live.find(
    (l) => l.pid && !claimed.has(l.pid) && samePath(l.cwd, pty.cwd) && l.startedAt >= pty.startedAt - PICK_START_SLACK_MS,
  )?.pid;
}
...
function createTerminalService(o) {
  ...
  function spawnSession(id, mode, cwdCandidate, cols, rows, spec) {
    ...
    const env = ptyEnv({ claudeDir: o.claudeDir, isDefaultDir: o.isDefaultDir, noFlicker: config.noFlicker });
  ...
  function watchPick(s) {
    s.pickTimer = setInterval(() => {
      ...
      clearInterval(s.pickTimer);
      if (sessions.has(picked)) return;
      sessions.delete(s.id);
      s.id = picked;
      s.mode = 'resume';
      sessions.set(picked, s);
      for (const ws of s.sockets) send(ws, { t: 'rekey', id: picked });
    }, PICK_POLL_MS);
  }
  ...
  function onHello(ws, msg) {
    ...
    if (mode === 'new' || mode === 'pick') {
      ...
      const dir = spec?.cwd ?? msg.cwd;
      if (typeof dir !== 'string' || !o.isAllowedFolder(dir)) {
...
module.exports = { createTerminalService, readTerminalConfig, shellArgs, resolveShell, claudeArgsFor, parseNewSpec, findPickProcess };

...
function extractGoalFromLine(data) {
  ...
  // `/goal clear` is a type:system local_command line carrying the command
  // markup in `content`; a user-typed command line carries it in message.content.
  const content = data.content ?? data.message?.content;
  if (typeof content === 'string' && content.includes(GOAL_COMMAND_TAG) && /<command-args>\s*clear/.test(content)) {
    return null;
  }
  return undefined;
}
...
const GOAL_MARKERS = ['"goal_status"', GOAL_COMMAND_TAG].map((m) => Buffer.from(m));
const SCAN_CHUNK = 65536;
const scanBuf = Buffer.allocUnsafe(SCAN_CHUNK);

// Streams [from, to) and calls onLine for the complete lines of every chunk
// that contains a marker. Chunks without one are never decoded, which keeps a
// full pass close to raw read speed (measured ~4x cheaper than decode+split).
function scanMarkedLines(fd, from, to, markers, onLine) {
  let carry = Buffer.alloc(0);
  let offset = from;
  while (offset < to) {
    const n = fs.readSync(fd, scanBuf, 0, Math.min(SCAN_CHUNK, to - offset), offset);
    if (n === 0) break;
    offset += n;
    const view = carry.length ? Buffer.concat([carry, scanBuf.subarray(0, n)]) : scanBuf.subarray(0, n);
    const cut = view.lastIndexOf(0x0a) + 1;
    const body = view.subarray(0, cut);
    carry = Buffer.from(view.subarray(cut));
    if (markers.some((m) => body.includes(m))) for (const line of body.toString('utf8').split('\n')) onLine(line);
  }
  if (carry.length && markers.some((m) => carry.includes(m))) onLine(carry.toString('utf8'));
}

// Goal events are rare and can sit anywhere in a long JSONL, outside both the
// head and tail windows. Returns the last goal update in [from, to), or
// undefined when there is none.
function scanGoalInRange(fd, from, to) {
  let latest;
  scanMarkedLines(fd, from, to, GOAL_MARKERS, (line) => {
    if (!line.includes('"goal_status"') && !line.includes(GOAL_COMMAND_TAG)) return;
    try {
      const update = extractGoalFromLine(JSON.parse(line));
      if (update !== undefined) latest = update;
    } catch (_) {}
  });
  return latest;
}
...
function readSessionInfoFromJsonl(jsonlPath) {
  const result = { slug: null, projectPath: null, cwd: null, gitBranch: null, customTitle: null, logicalParentUuid: null, compactBoundaryUuid: null, goal: null };
  let stat;
  let fd;
  try {
  ...
  try {
    ...
    if (canIncrement) {
      ...
      if (deltaLen > 0) {
        ...
        // The bytes an over-cap delta skips may hold a goal set, met or clear;
        // scanned first so the delta's own lines win by order.
        if (deltaLen > DELTA_MAX) {
          const skipped = scanGoalInRange(fd, cached.scannedUpTo, readStart);
          if (skipped !== undefined) result.goal = skipped;
        }
      ...
      if (stat.size > offset) {
        ...
        // A goal can be set, met or cleared between the head and tail windows.
        // The scan starts at the head's last partial line and runs to EOF, so a
        // tail event is simply seen again in order.
        const goal = tailGoal !== undefined ? tailGoal : scanGoalInRange(fd, offset - Buffer.byteLength(leftover), stat.size);
...
// The `scratch new` calls in a transcript: `{ ts, path }` per call, where `path` is the
// manifest the CLI printed. That output is the exact answer, so the command string is
// never parsed — a real invocation reads
//   cd '<repo>'; mkdir -p _scratchpads; scratch new "vid-promo" --dir _scratchpads ...
// and recovering the pad from it would mean tracking the `cd`, resolving a relative
// `--dir` and unquoting the name. `path` is null when the output never reached the
// transcript, which happens for real: that same command ended in `| tail -2`, cutting
// the manifest line off. The caller falls back to `ts` for those.
function readScratchpadCreations(jsonlPath) {
  const calls = new Map();
  try {
    const content = readFileSync(jsonlPath, 'utf8');
    for (const line of content.split('\n')) {
      ...
      for (const b of blocks) {
        if (b?.type === 'tool_use' && b.name === 'Bash' && SCRATCH_NEW_RE.test(b.input?.command || '')) {
          if (b.id && Number.isFinite(ts)) calls.set(b.id, { ts, path: null });

...
function readMessagesPage(jsonlPath, limit = 10, beforeTimestamp = null) {
  ...
  while (allMessages.length === readLimit && readLimit < 10000) {
    readLimit *= 4;
    allMessages = readRecentMessages(jsonlPath, readLimit);
  }

  if (beforeTimestamp) {
    allMessages = allMessages.filter(m => m.timestamp && m.timestamp < beforeTimestamp);
  }

  const page = allMessages.slice(-fetchLimit);

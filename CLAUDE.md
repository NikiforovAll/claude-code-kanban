...
- **The board can start sessions** — `dispatch start` (the `kanban-dispatch` skill) has cck start a Claude Code session in its embedded terminal, behind the terminal token (`lib/dispatch.js`). A started session is an ordinary session, not a child: the board shows no tree or status. Its placement is fixed from the first frame by a transient group (`lib/dispatch-groups.js`, `.cck/dispatch-groups.json`) and a placeholder from `dispatch-update`, so it never jumps into a group later. A report back is opt-in (`--report`). See `docs/dispatch.md`

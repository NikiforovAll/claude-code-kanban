...
The session has a fresh `_waiting.json` marker (< 30 min old, `PERMISSION_TTL_MS`) in its agent-activity directory. This covers both permission prompts (`PermissionRequest`) and user questions (`AskUserQuestion`). These sessions need attention even if they have no tasks or agents. The waiting check runs regardless of the session's staleness — even if the JSONL log is older than `AGENT_STALE_MS`, the waiting state is still evaluated. The marker is auto-resolved if the session's JSONL log was modified after the marker timestamp plus a grace period (`WAITING_RESOLVE_GRACE_MS`, 15s) — this handles cases where `PostToolUse` never fires (e.g. `ExitPlanMode`).

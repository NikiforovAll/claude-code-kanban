...
`scratchpadDir` is derived by convention from `meta.jsonlPath`, not looked up: `path.join(SCRATCHPAD_ROOT, path.basename(path.dirname(jsonlPath)), id, 'scratchpad')`, where `SCRATCHPAD_ROOT` is `<os.tmpdir()>/claude`. The harness creates the per-session dir lazily on first temp write, so a `statSync` here would both report "missing" for most sessions and add IO per session to the hot path. Pure string join — same cost as `projectDir`. Consequence: the path may not exist, and the UI's open-folder action fails with a toast in that case.

The one exception is a session whose project is a linked worktree (`resolveWorktree` non-null). A session launched with `claude -w` starts in the main checkout and enters the worktree afterwards, so the harness keys its scratchpad on the repo while the transcript is filed under the worktree — and nothing in the log records this, not even `cwd`, which reads as the worktree from the first entry. The two candidates (`<projEnc>` and `encodeProjectDirName(worktree.repo)`) are told apart by which one the harness created, so `getScratchpadDir` falls back to an `existsSync` pair for worktree sessions only: at most two stats, and only while neither dir exists, since the by-project answer short-circuits as soon as it does. Sessions started inside the worktree keep the by-project path, which is what they use. `encodeProjectDirName` mirrors the harness spelling of a path into a dir name: every non-alphanumeric character becomes a dash.

Listing what is *in* that dir is a separate, cold-path endpoint (`GET /api/sessions/:sessionId/scratchpad-files`), rendered as the file rows under the UI's Scratchpad path row. Clicking a row previews the file in the app: `/api/preview` now knows a third kind, `text`, for source and data extensions, rendered escaped in a `<pre>` and highlighted by extension, so the modal covers the `.py`/`.json`/`.js` files these dirs are full of. A 400 from that endpoint (an archive, an image, an unlisted extension like `.html.before`) is the client's signal to open the file in the editor instead. It returns every top-level file, any extension, newest first, and is deliberately non-recursive: sessions drop clones and build output in their scratchpad, and of 1302 real dirs measured the largest held 139k entries behind 64 top-level ones — an unpruned walk of it costs ~800 ms against 0.6 ms for the flat `readdir`. Top-level counts top out at 95 entries, so the endpoint returns the whole list and the client collapses it to three; expanding is a local re-render and costs no request. The per-entry `stat` the sort needs, not the `readdir`, is where the time goes — 95 of them cost 2.0 ms awaited one at a time against 0.9 ms fanned out through `Promise.all`, which is what the endpoint does. Nothing is cached server-side, so the client holds each list on a 10 s TTL; without it the zen panel, which re-renders on every SSE tick, would poll the dir every 2 s. Keep this off the session-list hot path: at 0.05 ms per dir it would add ~60 ms to every scan.

Recursion is a deliberate omission, not an oversight: notes written by the `scratch` skill live at `_scratchpads/<name>/` under whatever working folder the session used, so a session's markdown is often nested and absent from this list. A pruned depth-3 walk measures 0-3 ms on the same worst-case dirs, so the flat list is a scope choice about what the row means, not a performance ceiling.
...
`readScratchpadCreations` shares that shape and that `tool_use`→`tool_result` pairing, for `GET /api/sessions/:id/pads` — the pads a session made with `scratch new`, folded into the linked-documents list. It returns one `{ ts, path }` per call: `path` is the manifest the CLI printed, which names the pad outright, and is null when the command piped its output away (`scratch new … | tail -2` is a real transcript). `readCreatedPads` in `server.js` reads the reported paths directly and only falls back to a walk of `meta.project` for the unresolved calls, pairing a pad's `created` with a `ts` inside `PAD_CREATE_WINDOW_MS`. That walk is pruned and capped at depth 3, for the reason measured above: unpruned it costs ~800 ms on the worst-case project dirs. The cache is `padsByPath`, through the same `cachedByFileStat` helper as `artifactsByPath`; it stores the pending promise, so concurrent asks share one read. The rows are derived on every read and never stored. The client links each one into the ordinary linked-documents list, `preview-paths-<sessionId>`, exactly once — after that a pad row is indistinguishable from a hand-linked document, so the badge and the unlink button need no special case. `autolinked-pads-<sessionId>` records which pads that has already happened for, which is what stops a re-read of the transcript undoing an unlink.

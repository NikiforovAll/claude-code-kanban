...
`readScratchpadCreations` shares that shape and that `tool_use`→`tool_result` pairing, for `GET /api/sessions/:id/pads` — the pads a session made with `scratch new`, folded into the linked-documents list. It returns one `{ ts, path }` per call: `path` is the manifest the CLI printed, which names the pad outright, and is null when the command piped its output away (`scratch new … | tail -2` is a real transcript). `readCreatedPads` in `server.js` reads the reported paths directly and only falls back to a walk of `meta.project` for the unresolved calls, pairing a pad's `created` with a `ts` inside `PAD_CREATE_WINDOW_MS`. That walk is pruned and capped at depth 3, for the reason measured above: unpruned it costs ~800 ms on the worst-case project dirs. The cache is `padsByPath`, through the same `cachedByFileStat` helper as `artifactsByPath`; it stores the pending promise, so concurrent asks share one read. The rows are derived on every read and never stored, which is what makes re-reading a transcript idempotent — the only state that cannot be derived, which of the rows the user unlinked, lives in the client under `hidden-pads-<sessionId>` next to the hand-linked `preview-paths-<sessionId>`.

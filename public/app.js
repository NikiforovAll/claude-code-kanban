...
//#region MESSAGE_PANEL
      ...
      try {
        ...
        for (const m of data.messages) {
          if (m.agentId && m.agentPrompt) {
            ...
            if (agent && !agent.prompt) { agent.prompt = m.agentPrompt; agentEnriched = true; }
          }
        }
    ...
    function updateLatestMessage(messages) {
      ...
      for (let i = messages.length - 1; i >= 0; i--) {
        if (messages[i].type === 'assistant' || (messages[i].type === 'user' && !messages[i].systemLabel)) { last = messages[i]; break; }
      }
    ...
    function renderPinnedSection() {
      ...
      const chevron = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="12" height="12"><path d="M6 9l6 6 6-6"/></svg>';
      const items = currentPins.map((p, pi) => {
        ...
        if (p.type === 'user') {
          ...
          const msgTrunc = p.lastMessage ? escapeHtml(stripAnsi(p.lastMessage.trim()).replace(/[\r\n]+/g, ' ').slice(0, 60)) : '';
    ...
    function renderMessages(messages) {
      ...
      const msgsHtml = messages.map((m, i) => {
        ...
        if (m.type === 'user') {
          ...
          const agentLink = m.tool === 'Agent' && m.agentId ? ` <span class="msg-agent-link" title="View agent" onclick="event.stopPropagation();showAgentModal('${escapeHtml(m.agentId)}')">⇗</span>` : '';
          const itemClick = m.tool === 'Agent' && m.agentId
    ...
    const PIN_SVG = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="14" height="14"><path d="M19 21l-7-5-7 5V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2z"/></svg>';
    const MSG_ICON_USER = '<svg class="msg-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>';
    const MSG_ICON_ASSISTANT = '<svg class="msg-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="11" width="18" height="10" rx="2"/><circle cx="9" cy="16" r="1.5"/><circle cx="15" cy="16" r="1.5"/><path d="M12 2v4M8 7h8"/></svg>';
    const MSG_ICON_TOOL = '<svg class="msg-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14.7 6.3a1 1 0 0 0 0 1.4l1.6 1.6a1 1 0 0 0 1.4 0l3.77-3.77a6 6 0 0 1-7.94 7.94l-6.91 6.91a2.12 2.12 0 0 1-3-3l6.91-6.91a6 6 0 0 1 7.94-7.94l-3.76 3.76z"/></svg>';
    const MSG_ICON_SYSTEM = '<svg class="msg-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><line x1="12" y1="16" x2="12" y2="12"/><line x1="12" y1="8" x2="12.01" y2="8"/></svg>';

    function getPinId(m) {
      const content = m.type === 'tool_use'
        ? `${m.tool}:${(m.detail || '').slice(0, 100)}`
        : (m.text || '').slice(0, 100);
      return `${m.type}|${m.timestamp}|${content}`;
    }

    function loadPins(sessionId) {
      try { return JSON.parse(localStorage.getItem(`pinned-messages-${sessionId}`)) || []; }
      catch { return []; }
    }
    ...
    function isPinned(m) { return currentPins.some(p => p.id === getPinId(m)); }

    function isAgentPinned(agentId) { return currentPins.some(p => p.id === `agent|${agentId}`); }
    ...
    function toggleAgentPin(agentId) {
      ...
      if (idx >= 0) {
        ...
        currentPins.push({
          id, type: 'agent', agentId: agent.agentId,
    ...
    function togglePin(msgIndex) {
      ...
      if (idx >= 0) {
        ...
        pinnedCollapsed = false;
        currentPins.push({
          id, type: m.type, text: m.text || null, fullText: m.fullText || null,
          tool: m.tool || null, detail: m.detail || null, fullDetail: m.fullDetail || null,
          description: m.description || null, timestamp: m.timestamp, model: m.model || null,
          agentId: m.agentId || null, agentPrompt: m.agentPrompt || null,
    ...
    function showPinnedMsgDetail(pinIdx) {
      ...
      if (idx >= 0) { currentPinDetailId = null; showMsgDetail(idx); return; }
      ...
      if (pin.type === 'tool_use') {
        ...
        body.innerHTML = (fullText ? `<pre class="msg-detail-pre">${pinDetailRendered}</pre>` : '<em>No details</em>') + pinParamsHtml + pinResultHtml;
    ...
    function updateMsgDetailPinState() {
      ...
      if (currentMsgDetailIdx != null && currentMessages[currentMsgDetailIdx]) {
        ...
        pinBtn.classList.toggle('active', currentPins.some(p => p.id === currentPinDetailId));
      }
    }
...
//#region PINNING
    ...
    function loadPinnedSessions() {
      try { return new Set(JSON.parse(localStorage.getItem('pinned-sessions')) || []); }
...
//#region TOAST
    ...
    async function copyWithFeedback(text, btn) {
      ...
      try {
        ...
        btn.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="16" height="16"><path d="M20 6L9 17l-5-5"/></svg>';
        setTimeout(() => { btn.innerHTML = svg; delete btn.dataset.copying; }, 1500);
      } catch (e) {
...
//#region TOOL_RENDERING
    function renderToolParamsHtml(params) {
      ...
      for (const [k, v] of Object.entries(params)) {
        ...
        if (display.length > 60) {
          ...
          badges.push(`<span style="display:inline-flex;align-items:center;gap:3px;padding:1px 6px;border-radius:3px;background:var(--bg-secondary);font-size:0.75rem"><span style="color:var(--text-muted)">${escapeHtml(k)}:</span> ${escapeHtml(display)}</span>`);
        }
      }
      let html = '';
      ...
      if (params.content) {
        ...
        const truncContent = contentTruncated ? params.content.slice(0, CONTENT_TRUNCATE_MAX) + '\n... (truncated)' : params.content;
        let writeMoreBtn = '', fullBlock = '';
        if (contentTruncated) {
          const toggle = makeExpandToggle(escapeHtml(truncContent), escapeHtml(params.content), { fontSize: '0.75rem', maxHeight: '500px' });
    ...
    function highlightBash(escaped) {
      return escaped
        ...
        .replace(/\b(if|then|else|elif|fi|for|do|done|while|until|case|esac|function|return|in|select)\b/g, '<span style="color:#c586c0">$1</span>')
        .replace(/\b(echo|cd|ls|cat|grep|awk|sed|rm|cp|mv|mkdir|chmod|chown|export|source|exit|test|read|printf|set|unset|eval|exec|trap|wait|kill|sudo|apt|npm|npx|git|docker|curl|wget|pip|python|node|make|dotnet)\b/g, '<span style="color:#569cd6">$1</span>')
    ...
    function makeExpandToggle(truncatedHtml, fullHtml, opts = {}) {
...
//#region AGENTS
    function renderAgentFooter() {
      ...
      const permFresh = currentWaiting && currentWaiting.timestamp &&
        (now - new Date(currentWaiting.timestamp).getTime()) < WAITING_TTL_MS;
...
//#region RENDERING
    ...
    function renderSessions() {
      ...
      if (sessionFilter === 'active') {
        ...
        filteredSessions = filteredSessions.filter(s => {
          const isActive = s.hasMessages && (s.pending > 0 || s.inProgress > 0 || s.hasActiveAgents || s.hasWaitingForUser || s.hasRecentLog ||
        ...
        const planSessions = sessions.filter(s => s.planImplementationSessionId && activeSessionIds.has(s.planImplementationSessionId) && !activeSessionIds.has(s.id));
      ...
      const renderSessionCard = (session) => {
        ...
        const isLive = hasInProgress || (session.modifiedAt && (Date.now() - new Date(session.modifiedAt).getTime()) <= LIVE_INDICATOR_MS);
      ...
      if (sessionFilter === 'active') {
        ...
        if (newPaths.length > 1) {
          const maxTime = new Map(newPaths.map(p => [p, Math.max(...groups.get(p).map(s => new Date(s.modifiedAt).getTime()))]));
        ...
        for (const [projectPath, projectSessions] of sortedGroups) {
          ...
          const breadcrumbParts = projectPath.replace(/^\/home\/[^/]+/, '~').split(/[/\\]/).filter(Boolean);
          const breadcrumbHtml = breadcrumbParts.map((p, i) => i < breadcrumbParts.length - 1 ? `${escapeHtml(p)}<span class="sep">/</span>` : escapeHtml(p)).join('');
    ...
    function renderSession() {
      ...
      const displayName = session.customTitle || session.name || session.gitBranch || session.description || currentSessionId;
        ...
        function debouncedRefresh(sessionId, isMetadata) {
          if (isMetadata) {
            ...
            taskRefreshTimer = setTimeout(async () => {
              ...
              if (viewMode === 'all') {
                currentTasks = filterProject
                  ? allTasksCache.filter(t => matchesProjectFilter(t.project))
                  : allTasksCache;
                renderAllTasks();
                renderLiveUpdatesFromCache();
...
//#region SIDEBAR_LAYOUT
    ...
    function loadPanelWidths() {
      [['detail-panel', '--detail-panel-width'], ['message-panel', '--message-panel-width']].forEach(([id, cssVar]) => {
...
//#region SESSION_INFO
    ...
    function showInfoModal(session, teamConfig, tasks, planContent) {
      ...
      titleEl.innerHTML = escapeHtml(titleText) + (session.modifiedAt
      ...
      const clickableStyle = 'font-family: \'IBM Plex Mono\', monospace; font-size: 12px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; cursor: pointer; color: var(--accent-text); text-decoration: underline; text-decoration-style: dotted; text-underline-offset: 3px;';
      const plainStyle = 'font-family: \'IBM Plex Mono\', monospace; font-size: 12px; user-select: all; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;';
...
//#region INIT
    ...
    loadSidebarState();
    try { const cg = JSON.parse(localStorage.getItem('collapsedGroups') || '[]'); cg.forEach(p => collapsedProjectGroups.add(p)); } catch(_) {}
    initSidebarResize();
    loadPanelWidths();
